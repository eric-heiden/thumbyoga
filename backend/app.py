...
def upload():
	...
	if file and allowed_file(file.filename) != False:
		...
		dt = Image.open(target_filename)._getexif()[36867]
		print('Date', dt)
		dt = datetime.strptime(dt, '%Y:%m:%d %H:%M:%S')
		print('Parsed date', dt)
		filename = "%s.%s" % (dt.strftime('%Y%m%d%H%M%S'), extension)
		os.rename(target_filename, os.path.join(app.config['UPLOAD_FOLDER'], filename))

		# with open(target_filename, 'rb') as fh:
